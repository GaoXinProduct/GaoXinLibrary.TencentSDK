...
public sealed class IpRangeService
{
    ...
    {
        var resp = await _http.GetAsync<GetIpListResponse>("/cgi-bin/get_api_domain_ip", ct: ct).ConfigureAwait(false);
        return resp.IpList ?? [];
    }
    ...
    {
        var resp = await _http.GetAsync<GetIpListResponse>("/cgi-bin/getcallbackip", ct: ct).ConfigureAwait(false);
        return resp.IpList ?? [];
    }
}
