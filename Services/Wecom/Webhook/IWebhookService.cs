...
public interface IWebhookService
{
    ...
    Task SendAsync(string webhookKey, WebhookMessageRequest request, CancellationToken ct = default);

    /// <summary>
    /// 发送文本消息，支持 @指定成员
    /// </summary>
    /// <param name="webhookKey">群机器人 Webhook Key</param>
    ...
    /// <param name="ct">取消令牌</param>
    Task SendTextAsync(string webhookKey, string content, string[]? mentionedList = null, string[]? mentionedMobileList = null, CancellationToken ct = default);
    ...
    /// <param name="ct">取消令牌</param>
    Task SendMarkdownAsync(string webhookKey, string content, CancellationToken ct = default);
    ...
    /// <param name="ct">取消令牌</param>
    Task SendNewsAsync(string webhookKey, WebhookNewsArticle[] articles, CancellationToken ct = default);
    ...
    /// <param name="ct">取消令牌</param>
    Task SendImageAsync(string webhookKey, string base64, string md5, CancellationToken ct = default);
    ...
    /// <param name="ct">取消令牌</param>
    Task SendFileAsync(string webhookKey, string mediaId, CancellationToken ct = default);
}
