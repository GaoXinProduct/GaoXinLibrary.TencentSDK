...
/// <summary>
/// 微信公众号 JS-SDK jsapi_ticket 获取与缓存管理
/// <para>
...
///     <list type="bullet">
///       <item>配置 <c>TicketShareSecret</c> 后，可用 <see cref="GetSharedTicketAsync"/> 对外暴露加密 Ticket。</item>
///       <item>同时配置 <c>TicketShareUrl</c> 后，刷新时将从该地址获取加密 Ticket 并自动解密，而非直接请求微信 API。</item>
///     </list>
///   </item>
/// </list>
/// </para>
...
public class JsApiTicketProvider
{
    ...
    /// 初始化 Ticket 提供程序
    /// </summary>
    public JsApiTicketProvider(WechatHttpClient http, HttpClient rawHttpClient)
    {
        _http = http;
    ...
    /// <param name="ticketShareUrl">远端共享 Ticket 地址；设置后将从此地址获取加密 Ticket 而非直接请求微信 API</param>
    ...
    public async Task<string> GetTicketAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                newTicket = response.Ticket ?? throw new TencentException("jsapi_ticket 为空");
    ...
    /// <para>
    /// 供主服务对外暴露 Ticket 共享接口使用，需配置 <c>TicketShareSecret</c>。<br/>
    /// 返回的 <see cref="SharedTokenResult.ExpiresIn"/> 为主服务侧 Ticket 的剩余秒数，<br/>
    /// 备服务应将此值原样写入响应 JSON 的 <c>expires_in</c> 字段，
    /// 以便 SDK 在 <see cref="GetTicketAsync"/> 中据此设置本地缓存过期时间，避免与主服务不同步。
    /// </para>
    /// </summary>
    /// <exception cref="InvalidOperationException">未配置 TicketShareSecret 时抛出</exception>
