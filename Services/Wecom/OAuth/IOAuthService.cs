...
/// <para>
/// 企业微信 OAuth2 授权流程：<br/>
/// 1. 调用 <see cref="BuildAuthUrl"/> 构造授权链接，将用户重定向至该 URL；<br/>
/// 2. 用户同意授权后，企业微信回调 <c>redirect_uri</c> 并携带 <c>code</c> 参数；<br/>
/// 3. 服务端使用 <c>code</c> 调用 <see cref="GetUserInfoAsync"/> 换取用户身份；<br/>
/// 4. 若 scope 为 <see cref="OAuthScope.PrivateInfo"/>，可继续调用
///    <see cref="GetUserDetailAsync"/> 换取敏感字段（姓名、手机等）。<br/>
/// 对于企业微信 Web 登录（外部浏览器扫码/桌面端登录），请使用
/// <see cref="GetWebLoginUserInfoAsync"/> 获取用户登录身份。
/// </para>
/// </summary>
public interface IOAuthService
{
    ...
    string BuildAuthUrl(string redirectUri, int agentId = 0, string scope = OAuthScope.Base, string state = "");
    ...
    string BuildWebLoginUrl(string redirectUri, int agentId = 0, string state = "", string loginType = "CorpApp");
    ...
    /// </returns>
    Task<OAuthUserInfoResponse> GetUserInfoAsync(string code, CancellationToken ct = default);
    ...
    /// </returns>
    Task<WebLoginUserInfoResponse> GetWebLoginUserInfoAsync(string code, CancellationToken ct = default);
    ...
    Task<OAuthUserDetailResponse> GetUserDetailAsync(string userTicket, CancellationToken ct = default);
}
