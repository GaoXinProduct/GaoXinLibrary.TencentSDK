...
public sealed class SecurityService
{
    ...
    public async Task<GetFileLeakPreventionResponse> GetFileLeakPreventionAsync(CancellationToken ct = default)
        => await _http.PostAsync<GetFileLeakPreventionResponse>("/cgi-bin/security/get_file_oper_record", EmptyRequest.Instance, ct).ConfigureAwait(false);
    ...
    public async Task<GetDeviceInfoResponse> GetDeviceInfoAsync(GetDeviceInfoRequest request, CancellationToken ct = default)
        => await _http.PostAsync<GetDeviceInfoResponse>("/cgi-bin/security/trustdevice/get_by_user", request, ct).ConfigureAwait(false);
}
