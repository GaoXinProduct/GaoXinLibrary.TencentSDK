...
public sealed class AsyncImportService
{
    ...
    {
        var resp = await _http.PostAsync<AsyncImportJobResponse>("/cgi-bin/batch/syncuser", request, ct).ConfigureAwait(false);
        return resp.JobId;
    }
    ...
    {
        var resp = await _http.PostAsync<AsyncImportJobResponse>("/cgi-bin/batch/replaceuser", request, ct).ConfigureAwait(false);
        return resp.JobId;
    }
    ...
    {
        var resp = await _http.PostAsync<AsyncImportJobResponse>("/cgi-bin/batch/replaceparty", request, ct).ConfigureAwait(false);
        return resp.JobId;
    }
    ...
    public async Task<GetAsyncJobResultResponse> GetResultAsync(string jobId, CancellationToken ct = default)
        => await _http.GetAsync<GetAsyncJobResultResponse>("/cgi-bin/batch/getresult",
            new() { ["jobid"] = jobId }, ct).ConfigureAwait(false);
}
