...
public sealed class HrService
{
    ...
    {
        var resp = await _http.PostAsync<GetFieldConfigResponse>("/cgi-bin/hr/get_fields", EmptyRequest.Instance, ct).ConfigureAwait(false);
        return resp.Group ?? [];
    }
    ...
    {
        var resp = await _http.PostAsync<GetStaffInfoResponse>("/cgi-bin/hr/get_staff_info", request, ct).ConfigureAwait(false);
    ...
    public async Task UpdateStaffInfoAsync(UpdateStaffInfoRequest request, CancellationToken ct = default)
        => await _http.PostAsync<WecomBaseResponse>("/cgi-bin/hr/update_staff_info", request, ct).ConfigureAwait(false);
}
