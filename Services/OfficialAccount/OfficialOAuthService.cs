...
public class OfficialOAuthService : IOfficialOAuthService
{
    ...
    public string BuildAuthUrl(string redirectUri, string scope = OfficialOAuthScope.Base, string state = "")
    {
        ...
        return $"{TencentConstants.OfficialOAuthBaseUrl}/connect/oauth2/authorize?appid={_appId}&redirect_uri={encodedUri}&response_type=code&scope={scope}&state={Uri.EscapeDataString(state)}#wechat_redirect";
    }
