...
public sealed class CollectFormService
{
    ...
    public async Task<CreateCollectFormResponse> CreateCollectFormAsync(CollectFormOperationRequest request, CancellationToken ct = default)
        => await _http.PostAsync<CreateCollectFormResponse>("/cgi-bin/wedoc/create_collect", request, ct).ConfigureAwait(false);
    ...
    public async Task ModifyCollectFormAsync(CollectFormOperationRequest request, CancellationToken ct = default)
        => await _http.PostAsync<WecomBaseResponse>("/cgi-bin/wedoc/modify_collect", request, ct).ConfigureAwait(false);
    ...
    {
        var resp = await _http.PostAsync<GetCollectFormResponse>("/cgi-bin/wedoc/get_collect", request, ct).ConfigureAwait(false);
    ...
    public async Task<GetCollectAnswerResponse> GetCollectAnswerAsync(GetCollectAnswerRequest request, CancellationToken ct = default)
        => await _http.PostAsync<GetCollectAnswerResponse>("/cgi-bin/wedoc/get_form_answer", request, ct).ConfigureAwait(false);
}
