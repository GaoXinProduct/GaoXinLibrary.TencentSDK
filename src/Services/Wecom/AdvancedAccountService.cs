...
public sealed class AdvancedAccountService
{
    ...
    public async Task AssignAsync(AdvancedAccountOperationRequest request, CancellationToken ct = default)
        => await _http.PostAsync<WecomBaseResponse>("/cgi-bin/advanced_account/assign", request, ct).ConfigureAwait(false);
    ...
    public async Task CancelAsync(AdvancedAccountOperationRequest request, CancellationToken ct = default)
        => await _http.PostAsync<WecomBaseResponse>("/cgi-bin/advanced_account/cancel", request, ct).ConfigureAwait(false);
    ...
    public async Task<GetAdvancedAccountListResponse> GetListAsync(GetAdvancedAccountListRequest request, CancellationToken ct = default)
        => await _http.PostAsync<GetAdvancedAccountListResponse>("/cgi-bin/advanced_account/get_list", request, ct).ConfigureAwait(false);
}
